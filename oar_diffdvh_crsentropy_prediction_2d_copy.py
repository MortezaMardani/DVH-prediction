...
num=2835
output_size = 10
mesh_tnsr = np.zeros((num,50,50,11))  #np.zeros((num,50,50,33))
...
biases = {'bc1': tf.Variable(0.00001*tf.random_normal([num_filters_layer_1])),
          ...
          'bc3': tf.Variable(0.00001*tf.random_normal([num_filters_layer_3])),
          'bd1': tf.Variable(0.00001*tf.random_normal([num_features])),
          'bd2': tf.Variable(0.00001*tf.random_normal([output_size])),
}
...
for j in range(1000):
     ...
     step_size = step_size*0.95 #/(j+1)  #0.001/(j/10+1)
